...
class SpinState():
    ...
    def __init__(self, size, D, D_c, scan_time, TYPE=tf.float32):
        ...
        def gen_place_holder(i, j):
            ...
            return Node(tf.placeholder(self.TYPE, shape=[2, *[self.D for i in legs]]), ['p', *legs])
        self.lattice = [[gen_place_holder(i, j) for j in range(m)] for i in range(n)]
        self.state = tf.placeholder(tf.int32, [n, m])
        self.lat = [[self.lattice[i][j][self.state[i][j]] for j in range(m)] for i in range(n)]
        self.D_c = D_c
    ...
    def cal_E_s_and_Delta_s(self, cal_grad=True):
        ...
        E_s_non_diag = Node(0.)  # 为相邻两个交换后的w(s)之和
            ...
            for j in range(m-1):
                if self.state[i][j] != self.state[i][j+1]:
                    E_s_non_diag += l[(j-1) % m]\
                        .tensor_contract(self.UpToDown[(i-1) % n][j], ['r1'], ['l'], {}, {'r': 'r1'}, restrict_mode=False)\
                        .tensor_contract(self.lattice[i][j][1-self.state[i][j]], ['r2', 'd'], ['l', 'u'], {}, {'r': 'r2'}, restrict_mode=False)\
                        .tensor_contract(self.DownToUp[(i+1) % n][j], ['r3', 'd'], ['l', 'u'], {}, {'r': 'r3'}, restrict_mode=False)\
                        .tensor_contract(self.UpToDown[(i-1) % n][(j+1) % m], ['r1'], ['l'], {}, {'r': 'r1'}, restrict_mode=False)\
                        .tensor_contract(self.lattice[i][(j+1) % m][1-self.state[i][(j+1) % m]], ['r2', 'd'], ['l', 'u'], {}, {'r': 'r2'}, restrict_mode=False)\
            ...
            for i in range(n-1):
                if self.state[i][j] != self.state[i+1][j]:
                    E_s_non_diag += u[(i-1) % n]\
                        .tensor_contract(self.LeftToRight[i][(j-1) % m], ['d1'], ['u'], {}, {'d': 'd1'}, restrict_mode=False)\
                        .tensor_contract(self.lattice[i][j][1-self.state[i][j]], ['d2', 'r'], ['u', 'l'], {}, {'d': 'd2'}, restrict_mode=False)\
                        .tensor_contract(self.RightToLeft[i][(j+1) % m], ['d3', 'r'], ['u', 'l'], {}, {'d': 'd3'}, restrict_mode=False)\
                        .tensor_contract(self.LeftToRight[(i+1) % n][(j-1) % m], ['d1'], 'u', {}, {'d': 'd1'}, restrict_mode=False)\
                        .tensor_contract(self.lattice[(i+1) % n][j][1-self.state[(i+1) % n][j]], ['d2', 'r'], ['u', 'l'], {}, {'d': 'd2'}, restrict_mode=False)\
